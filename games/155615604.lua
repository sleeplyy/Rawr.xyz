                    ...
                    if check then
                        ...
                        for _, ent in entities do
                            if not ent.Character:GetAttribute('Arrested') then
                                ...
                                local canArrest = ent.Player.Team == criminalsTeam

                                if ent.Player.Team == inmatesTeam then
                                    canArrest = ent.Character:GetAttribute('Hostile') or ent.Character:GetAttribute('Trespassing')
                                end

                                if ent.Player.Team == guardsTeam or ent.Player.Team == neutralTeam then
                                    canArrest = false
                                end

                                if canArrest then
                                    local success = replicatedStorage.Remotes.ArrestPlayer:InvokeServer(ent.Player, 1)
                                    if success then
                                        cooldown = os.clock() + 7
